...
class IReDocStore(IDBDocStore):
    ...
    async def telegramUserAddPhoto(self, user_photo):
        async with await rdb.connect(host=self.host, port=self.port) as connection:
            await rdb.db(self.db).table("users").filter(
                {"tg_user_id": user_photo["tg_user_id"]}
            ).update(user_photo).run(connection)

    async def userAddPhoto(self, user_id, file_path):
        async with await rdb.connect(host=self.host, port=self.port) as connection:
            await rdb.db(self.db).table("users").filter({"user_id": user_id}).update(
                {"file_path": file_path}
            ).run(connection)
    ...
    async def telegramUserAddInfo(self, user_info):
        async with await rdb.connect(host=self.host, port=self.port) as connection:
            await rdb.db(self.db).table("users").filter(
                {"tg_user_id": user_info["tg_user_id"]}
            ).update(user_info).run(connection)
        ...
        async with await rdb.connect(host=self.host, port=self.port) as connection:
            ...
            if existing_reaction:
                await rdb.db(self.db).table("post_reaction").filter(
                    (rdb.row["post_id"] == user_reaction["post_id"])
                    & (rdb.row["user_id"] == user_id)
                ).update({"reaction_type": user_reaction["reaction_type"]}).run(
                    connection
                )
            else:
                await rdb.db(self.db).table("post_reaction").insert(user_reaction).run(
                    connection
                )
    ...
    async def sendReactionByUser(self, user_id, user_reaction):
        async with await rdb.connect(host=self.host, port=self.port) as connection:
            ...
            await rdb.db(self.db).table(table_name).insert(user_reaction).run(
                connection
            )
    ...
    async def messages(self, user_id, chat_user_id, page, limit):
        async with await rdb.connect(host=self.host, port=self.port) as connection:
            return (
                ...
                .table("messages")
                .filter(
                    (r.row["sender_id"] == user_id)
                    & (r.row["receiver_id"] == chat_user_id)
