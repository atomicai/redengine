    ...
    for p in data["posts"]:
        ...
        if p["type"] == "movie":
            ...
            keywords = (
                ...
                .table("keywords")
                .filter(
                    lambda keyword: rdb.expr(p["keywords"]).contains(keyword["word"])
                )
                .run(conn)
            )
            ...
            keyphrases = (
                ...
                .table("keyphrases")
                .filter(
                    lambda keyphrase: rdb.expr(p["keyphrases"]).contains(
                        keyphrase["phrase"]
                    )
                )
                .run(conn)
            ...
            rdb.db("meetingsDb").table("posts").insert(
                {
                    "context": p["content"],
            ...
            keywords = (
                rdb.db("meetingsDb")
                .table("keywords")
                .filter(lambda keyword: rdb.expr(words).contains(keyword["word"]))
                .run(conn)
            )
            ...
            keyphrases = (
                ...
                .table("keyphrases")
                .filter(
                    lambda keyphrase: rdb.expr(phrases).contains(keyphrase["phrase"])
                )
                .run(conn)
            )
