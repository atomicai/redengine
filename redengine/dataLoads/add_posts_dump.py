...
with open(data_dir / filename) as json_file:
    ...
    for p in data["posts"]:
        if p["type"] == "movie":              
            ...
            print()
            rdb.db('meetingsDb').table('posts').insert({'context': p["content"],'type': p['type'],'movie_id': str(movie["id"]),'speaker_id': str(speaker["id"]), 'has_image':p["has_image"], 'img_path':p["img_path"],'keywords_ids': keywords_ids,'keyphrases_ids': keyphrases_ids}).run(conn)
        elif p["type"] == "book":
            if "keyphrases" not in p:
                p["keyphrases"] = []
            if "keywords" not in p:
            ...
            if book and author:
                rdb.db('meetingsDb').table('posts').insert({'context': p["content"], 'book_id': str(book["id"]),'author_id': str(author["id"]),
                                                          'has_image':p["has_image"],'type': p['type'], 'img_path':img_path,'keywords_ids': keywords_ids,'keyphrases_ids': keyphrases_ids}).run(conn)