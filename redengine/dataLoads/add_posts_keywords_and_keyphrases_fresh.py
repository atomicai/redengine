...
with open(data_dir / filename) as json_file:
    ...
    for p in data["posts"]:
        for word in p["keywords"]:
            repeat_words = rdb.db('meetingsDb').table('keywords').filter({'word': word["keyword_or_phrase"]}).nth(0).default(None).run(conn)

            if not repeat_words:
                    rdb.db('meetingsDb').table('keywords').insert({'word': word["keyword_or_phrase"], 'explanation': word['explanation']}).run(conn)
