...
with open(data_dir / filename) as json_file:
    ...
    for p in data["posts"]:
        for word in p["keywords"]:
            repeat_words = rdb.db('meetingsDb').table('keywords').filter({'word': word}).nth(0).default(None).run(conn)

            if not repeat_words:
                    rdb.db('meetingsDb').table('keywords').insert({'word': word}).run(conn)
        for phrase in p["keyphrases"]:
            repeat_phrase = rdb.db('meetingsDb').table('keyphrases').filter({'phrase': phrase}).nth(0).default(None).run(conn)

            if not repeat_phrase:
                    rdb.db('meetingsDb').table('keyphrases').insert({'phrase': phrase}).run(conn)