...
with open(data_dir / filename) as json_file:
    ...
    for p in data:
        rdb.db('meetingsDb').table('emojis').insert({'label': p,"value":data[p],"active":False}).run(conn)
    rdb.db('meetingsDb').table('emojis').filter(lambda emoji: rdb.expr(emojis).contains(emoji['label'])).update({"active":True}).run(conn)
