...
with open(data_dir / filename) as json_file:
    ...
    for p in data["posts"]:
        if "speaker" not in p:
            p["speaker"]=None
        repeat = rdb.db('meetingsDb').table('movie_speakers').filter({'name_speaker':p["speaker"]}).nth(0).default(None).run(conn)

        if not repeat:
            if "type" in p and p["type"]=='movie':
                rdb.db('meetingsDb').table('movie_speakers').insert({'name_speaker': p["speaker"]}).run(conn)
